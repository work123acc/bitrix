...
<div id="api_auth_register_<?=$arResult['FORM_ID']?>" class="api_auth_register" data-css="<?=$dataCss?>" data-js="<?=$dataJs?>">
	...
	class="api_form">
		...
		<? /*------------------------ Добавляем поля-параметры, т к в визуалки их не добавить ----------------------------------*/ ?>		
		<?
			$arParams["SHOW_FIELDS"] = array('UF_USER_INN', 'LOGIN', 'NAME', 'EMAIL', 'PASSWORD', 'CONFIRM_PASSWORD', 'PERSONAL_CITY', 'PERSONAL_PHONE', 'PERSONAL_STREET', 'WORK_COMPANY' );
			$arParams["REQUIRED_FIELDS"] = array('UF_USER_INN', 'LOGIN', 'NAME', 'EMAIL', 'PASSWORD', 'CONFIRM_PASSWORD');
		...
		<? if($arParams['SHOW_FIELDS']) { ?>
			<? foreach($arParams["SHOW_FIELDS"] as $FIELD) { ?>
				...
				<? if ($FIELD === 'UF_USER_INN' ) { $placeholder = 'ИНН' . (in_array($FIELD, $arParams["REQUIRED_FIELDS"]) ? ' *' : '') ; } ?>
