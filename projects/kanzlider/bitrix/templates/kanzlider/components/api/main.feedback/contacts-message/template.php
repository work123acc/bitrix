<?
	...
	*/
	...
	if($arParams['HIDE_FORM_AFTER_SEND'] && $arResult['MESSAGE']['SUCCESS'])
	$bHideForm = true;
...
<script type="text/javascript">
	...
	throw new Error("#<?= $FORM_ID ?> requires jQuery");
	...
	jQuery(document).ready(function ($) {
		...
		$('#<?= $FORM_ID ?> .api-tooltip').tooltipster({
			<?if($arParams['TOOLTIPSTER_OPTIONS']):?>
			<?foreach($arParams['TOOLTIPSTER_OPTIONS'] as $option):?>
			<?=$option?>,
			<?endforeach;?>
			<?endif?>
		});
		...
		*/);
		...
		var modal_<?= $FORM_ID ?> = '<div class="modal--show" id="modal<?= $FORM_ID ?>" tabindex="-1" role="dialog" aria-labelledby="modal-label" aria-hidden="true">' +
		'<div class="modal-inner">' +
		'<div class="modal-header"><?=htmlspecialcharsback(CUtil::JSEscape($arParams["CSS_MODAL_HEADER"]))?></div>' +
		'<div class="modal-content"><?=htmlspecialcharsback(CUtil::JSEscape($arParams["CSS_MODAL_CONTENT"]))?></div>' +
		'<div class="modal-footer"><?=htmlspecialcharsback(CUtil::JSEscape($arParams["CSS_MODAL_FOOTER"]))?></div>' +
		'</div>' +
		'<a href="#!" class="modal-close" title="<?=GetMessage("CLOSE_CSS_MODAL_WINDOW")?>" data-close="<?=GetMessage("CLOSE_CSS_MODAL_WINDOW")?>" data-dismiss="modal">&times;</a>' +
		'</div>';
		
		$('body').append(modal_<?= $FORM_ID ?>);
		window.location.hash = '#modal<?=$FORM_ID?>';
		<? endif; ?>
		
		<?if($arParams['USE_AGREEMENT']):?>
		if ($('#<?=$FORM_ID?> .group-agreement').length) {
			$('#API_MF_SUBMIT_BUTTON_<?=$FORM_ID?>').click(function () {
...
<div id="API-MF-<?=$FORM_ID?>" class="<?=$arParams['FORM_CLASS']?> api-mainfeedback tpl-default tpl-default-<?=$template_style;?>">
	<?=$arResult['FORM_TITLE']?>
	...
	<div class="uk-form-row api-ok-text-after"><?=$arParams['OK_TEXT_AFTER'];?></div>
	<? endif; ?>
	...
	autocomplete="<?=$FORM_AUTOCOMPLETE?>">
		...
		<input type="text" name="ANTIBOT[NAME]" value="<?=$arResult['ANTIBOT']['NAME'];?>" class="api-mf-antibot">
		<? endif; ?>
		<?
			...
			if(count($arParams['DISPLAY_FIELDS']) > 0)
			{
				foreach($arParams['DISPLAY_FIELDS'] as $FIELD)
				{
					...
					if ($FIELD === 'AUTHOR_NAME') {
						...
						} else if ($FIELD === 'AUTHOR_MESSAGE') {
						$thisID = 'input-message';
						$placeholder = "Текст сообщения";
					}
					//my_dump( array ($FIELD, $thisID, $placeholder) );
					...
					<div class="input-block--required cart__information-input-block">
						...
						<? if(!$arParams['HIDE_FIELD_NAME']) { ?>
							<span class="cart__information-input-desc"><?=$INPUT_NAME?></span>
						<? } ?>
		...
		<?
			...
			foreach($arParams['CUSTOM_FIELDS'] as $fk => $FIELD)
			{
				...
				$tooltip = $tooltipTitle = $tooltipContent = '';
				...
				foreach($arFields as $arField)
				{
					...
					elseif(substr($arField, 0, 7) == "tooltip")
					{
						$tooltip = true;
						if(substr($arField, 0, 13) == "tooltip-title")
						{
							$arData = explode('=', $arField);
							if($arData[1])
							$tooltipTitle = CUtil::JSEscape(htmlspecialcharsback($arData[1]));
						}
						
						if(substr($arField, 0, 15) == "tooltip-content")
						{
							$arData = explode('=', $arField);
							if($arData[1])
							$tooltipContent = CUtil::JSEscape(htmlspecialcharsback($arData[1]));
						}
					}
				}
				unset($arData);
				...
				if($arParams['INCLUDE_TOOLTIPSTER'] && $tooltip)
				{
					$tooltipOptions = '';
					if(strlen($tooltipTitle)>0)
					$tooltipOptions .= 'title="'. $tooltipTitle .'" ';
					
					if(strlen($tooltipContent)>0)
					$tooltipOptions .= 'data-tooltip-content="'. $tooltipContent .'" ';
					
					$INPUT_TOOLTIP = '<span class="api-tooltip" '.  $tooltipOptions .'></span>';
				}
				...
				<div class="<?=$INPUT_ROW_CLASS?>">
					...
					<div class="<?=$INPUT_CONTROLS_CLASS?>">
								...
								if($optgroup)
								{
									foreach($values as $k2 => $v2)
									{
										...
										else
										{
											...
											foreach($arValues as $val)
											{
												...
												$selected = false;
											?>
											<option<? if($selected): ?> selected<? endif; ?> value="<?=$val;?>"><?=$val;?></option>
											<?
												if($val == end($arValues))
												echo '</optgroup>';
											}
									...
									else
									{
										foreach($values as $k1 => $v)
										{
											...
											$selected = false;
										?>
										<option<? if($selected): ?> selected<? endif; ?> value="<?=$v?>"><?=$v?></option><?
										}
									}
								?>
							</select>
						...
						<div class="<?=$INPUT_ROW_CLASS?><? if($value):?> one-checkbox<?endif; ?>"  id="<?=$rowId?>">
							...
							<label class="<?=$INPUT_LABEL_CLASS?>"><?=$INPUT_NAME?></label>
							<?endif; ?>
							<div class="<?=$INPUT_CONTROLS_CLASS?> uk-form-controls-text<? if($values):?> group-checkbox<?endif; ?>">
								<div class="<?=$values ? $INPUT_CLASS : ''?>">
									<? if($values):?>
									<? if($arParams['HIDE_FIELD_NAME']):?>
						...
						<div class="<?=$INPUT_ROW_CLASS?>" id="<?=$rowId?>">
							...
							<label class="<?=$INPUT_LABEL_CLASS?>"><?=$INPUT_NAME?></label>
							<?endif; ?>
							<div class="<?=$INPUT_CONTROLS_CLASS?> uk-form-controls-text group-radio">
			...
			<?
				...
				if(count($arParams['DISPLAY_FIELDS']) > 0)
				{
					foreach($arParams['DISPLAY_FIELDS'] as $FIELD)
					{
						...
						<div class="input-block--required cart__information-input-block  cart__information-input-block--textarea">
			...
			<div class="api-mf-captcha <?=$INPUT_ROW_CLASS?>">
				...
				<div class="<?=$INPUT_CONTROLS_CLASS?>">
					<div class="api-mf-captcha-inner">
						...
						autocomplete="off"
						class="<?=$INPUT_CLASS;?>">
						<img src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult['capCode']?>" width="180" height="40" alt="CAPTCHA CODE">
					</div>
					<?=$INPUT_MESSAGE_DANGER;?>
				</div>
			</div>
			...
			<div id="API-MF-AGREEMENT-<?=$FORM_ID?>" class="<?=$INPUT_ROW_CLASS?> group-agreement">
				...
				<? endif; ?>
				<div class="<?=$INPUT_CONTROLS_CLASS?>">
					<input id="API_MF_AGREEMENT_<?=$FORM_ID?>" type="checkbox" name="API_MF_AGREEMENT" value="Y"<? if($arResult['API_MF_AGREEMENT'] == 'Y'): ?> checked=""<? endif; ?>>&nbsp;<!--
			...
			<div class="<?=$INPUT_ROW_CLASS?> group-button">
				...
				<div class="contacts__input-submit">
					<input type="submit" value="<?=$arResult['FORM_SUBMIT_VALUE'];?>">
				</div>
