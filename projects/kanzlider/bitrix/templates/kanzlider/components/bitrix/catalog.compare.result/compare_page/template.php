					...
					foreach ($arResult["SHOW_FIELDS"] as $code => $arProp)
					{
						...
						if ($showRow)
						{
							?><tr><td><?=GetMessage("IBLOCK_FIELD_".$code)?></td><?
								...
								<?
									/*------------------------------- Получение url через getlist -----------------------------------------*/
									
									$a = CIBlockElement::GetList( Array("SORT"=>"ASC"), Array('IBLOCK_ID'=>15, 'NAME'=>$arElement['NAME']),false, false, Array('ID', 'NAME', 'CODE', 'IBLOCK_SECTION_ID') );
									
									while ($b = $a->GetNext()) {
										$sect_id = intval($b['IBLOCK_SECTION_ID']);
										$el_url = $b['CODE']. '.html';
									}
									
									$a = CIBlockSection::GetList( Array("SORT"=>"ASC"), Array('IBLOCK_ID'=>15, 'ID'=>$sect_id),false,Array('ID', 'NAME', 'CODE'),false );
									while ($b = $a->GetNext()) {
										$url = '/catalog/' . $b['CODE'] . '/' . $el_url;
									}
								?>
								...
								<td valign="top">
									...
									<?
										...
										<div>
											<a href="<?=$url ?>">
												<img
												border="0" src="<?=$arElement["PREVIEW_PICTURE"]["SRC"]?>"
												width="auto" height="150"
												alt="<?=$arElement["FIELDS"]["PREVIEW_PICTURE"]["ALT"]?>" title="<?=$arElement["FIELDS"]["PREVIEW_PICTURE"]["TITLE"]?>"/>
											</a>
										</div>
										
										<a href="<?=$url?>"><?=$arElement[$code]?></a>
