	...
	*/
	...
	if ( $arResult['ITEMS'] ) {
		foreach ($arResult['ITEMS'] as $item) {
			if ($item['PROPERTIES']['INDEX_SPEC']['~VALUE'] === 'Y') {
		?>
		...
		<article class="goods__item">
			...
			<div class="goods__description">
				<div class="goods__count-box">
					<div class="goods__count-package">
						<?= $item["PROPERTIES"]["UNIT_PACKAGING_1C"]["VALUE"] ?> шт. в уп.
					</div>
				...
				<div class="goods__code-box">
					...
					<div class="goods__code">
						<?$kod = htmlentities($item["PROPERTIES"]["CODE_1C"]["~VALUE"]);
						$kod = str_replace("&nbsp;",'',$kod);?>
						<? echo $kod;?>
			...
			<div class="goods__price-box">
				<div class="goods__count-select">
					<input type="text" value="<?=$item["PROPERTIES"]["UNIT_PACKAGING_1C"]["VALUE"]?>" class="quantity_input">
				</div>
				<div class="goods__count-but-box">
					<input type="hidden" class="product_kol" value="<?=$item["OFFERS"][0]["CATALOG_QUANTITY"]?>" />
					<input type="hidden" class="shag" value="<?=$item["PROPERTIES"]["UNIT_PACKAGING_1C"]["VALUE"]?>" />
			...
			<div class="goods__advanced-box">
				<div class="goods__advanced-button  goods__advanced-button--compare">
					...
					<? if (isset($_SESSION["CATALOG_COMPARE_LIST"][16]["ITEMS"][$item["OFFERS"][0]["ID"]])) { ?>
					<a href="/catalog/compare/" id="compare_<?=$item["OFFERS"][0]["ID"]?>"/>В сравнении</a>					
					<? } else { ?>
				<a href="javascript:void(0)" id="compare_<?=$item["OFFERS"][0]["ID"]?>" onclick="docomparenovinki(<?=$item["OFFERS"][0]["ID"]?>)" />сравнить</a>				
			<? } ?>
