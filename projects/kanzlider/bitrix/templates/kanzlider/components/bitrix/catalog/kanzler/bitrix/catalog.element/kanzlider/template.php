...
<div class="card__product">
	...
	<div class="card__product-left">
			...
			<?if (!empty($actualItem['MORE_PHOTO']))
				{?><div class="card__image-images">
                    <?foreach ($actualItem['MORE_PHOTO'] as $key => $photo)
						...
						<img class="minimized"  src="<?=$photo['SRC']?>" alt="<?=$alt?>" title="<?=$title?>"<?=($key == 0 ? ' itemprop="image"' : '')?>>
		...
		<div class="card__product-advantage-buttons">
			...
			<? if ( isset($_SESSION["CATALOG_COMPARE_LIST"][16]["ITEMS"][ $arResult["OFFERS"][0]["ID"] ] ) ) { ?>
				<a href="/catalog/compare/" id="compare_<?=$arResult["OFFERS"][0]["ID"]?>" class="adv__button card__product-advantage-button  card__product-advantage-button--compare">
					в сравнении
				</a>
				
				<? } else { ?>
				<a href="javascript:void(0)" id="compare_<?=$arResult["OFFERS"][0]["ID"]?>" class="adv__button card__product-advantage-button  card__product-advantage-button--compare" onclick="docomparedetail(<?=$arResult["OFFERS"][0]["ID"]?>)">
					cравнить
				</a>
				
			<? } ?>
			
			<a href="" class="adv__button card__product-advantage-button  card__product-advantage-button--favorite js-wishlist" data-wishiblock="<?=$arResult["IBLOCK_ID"]?>" data-wishid="<?=$arResult["ID"]?>">добавить в избранное</a>
		</div>
	</div>
	<div class="card__product-right">
		<?if($arResult["PROPERTIES"]["CML2_TRAITS"]["VALUE"][3]){?>
		<div class="card__product-logo">
                    <?
//                echo "<pre>";
//                print_r($arResult["PROPERTIES"]["CML2_TRAITS"]["VALUE"][3]);
//                echo "</pre>";
                    ?>
                    <span><?=$arResult["PROPERTIES"]["CML2_TRAITS"]["VALUE"][3]?></span>
		</div>
		<?}?>
		<div class="card__product-code">
                    <?//$str = str_replace('&nbsp;', ' ', $arResult["PROPERTIES"]["CODE_1C"]["VALUE"]);?>
			код:
			<span>
				<?//=preg_replace('/\s+/', ' ',$str );?>
				<?$kod = htmlentities($arResult["PROPERTIES"]["CODE_1C"]["VALUE"]);
				$kod = str_replace("&nbsp;",'',$kod);?>
				<? echo $kod;?>
			</span>
		</div>
		...
		<div class="card__product-characters">
			...
			<div class="card__product-char">
				Количество в упаковке <b><?=$arResult["PROPERTIES"]["UNIT_PACKAGING_1C"]["VALUE"]?></b> шт.
			</div>
			...
			<? /*------------------------------- comment -----------------------------------------*/ ?>
			<div class="card__product-char">
				Минимальная партия <b><?=$arResult["PROPERTIES"]["UNIT_PACKAGING_1C"]["VALUE"]?></b> шт.
			</div>
			<div class="card__product-char">
                            <?$pr=str_replace(',','.',$arResult["PROPERTIES"]["PRICE_MAX"]["VALUE"]);?>
				Цена с максимальной скидкой <b><?=round($pr,2)?> руб.</b>
			</div>
		...
		<div class="card__product-price-block">
			...
			<input type="text" id="quantity_input" min="<?= intval($arResult["PROPERTIES"]["UNIT_PACKAGING_1C"]["VALUE"]) ?>" class="card__product-count" value="<?= intval($arResult["PROPERTIES"]["UNIT_PACKAGING_1C"]["VALUE"]) ?>">
			...
			<div class="card__product-price"><span><?=$price["RATIO_PRICE"]?></span>Р</div>
