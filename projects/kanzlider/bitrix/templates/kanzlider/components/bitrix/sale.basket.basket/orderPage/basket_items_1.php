				...
				if ($arHeader["id"] == "NAME"):
					...
					?>
						...
						if ($bPropsColumn):
						    ...
						    foreach ($arItem["PROPS"] as $valDlya) {
							if ($valDlya["CODE"] == "DLYA_KOGO") {
							    switch ($valDlya["VALUE"]) {
								case "Не выбранно": $x = 0;
								    break;
								case "Мальчик": $x = 1;
								    break;
								case "Девочка": $x = 2;
							...
							if ($val["CODE"] == "MALCHIK_DEVOCHKA" and $val["VALUE"] == "Да") {
							    ?>
							    <?
//echo "<pre>";
//print_r($val);
//echo "</pre>";
							    ?>
							    <span>Выберите мальчик или девочка</span><br />
							    <select class="cart_select">
								<option data-cart="<?= $val["BASKET_ID"] ?>" value="0" <? if ($x == 0) echo "selected" ?>>Не выбранно</option>
								<option data-cart="<?= $val["BASKET_ID"] ?>" value="1" <? if ($x == 1) echo "selected" ?>>Мальчик</option>
								<option data-cart="<?= $val["BASKET_ID"] ?>" value="2" <? if ($x == 2) echo "selected" ?>>Девочка</option>
							    </select>  
							    <span class="select_text">
								Для выбранно цвета могут быть разные варианты рисунка.</span>
							    <span class="select_text">Выберите пол ребёнка и мы подбирем подходяший рисунок. 
							    </span>
			    <?
			    }else {
				if ($val["CODE"] != "DLYA_KOGO" and $val["CODE"] != "TSVETA") {
				    if ($val["CODE"] == "RAZMER") {
					echo "Размер:&nbsp;<span>" . $val["VALUE"] . "</span><br/>";
				    } else {
					if ($val["CODE"] != "MALCHIK_DEVOCHKA") {
		...
		elseif ($arHeader["id"] == "QUANTITY"):
			...
			?>
				    ...
				    <td class="control">
					    ...
					    if ($bDeleteColumn):
						?>
		    			<a href="<?= str_replace("#ID#", $arItem["ID"], $arUrls["delete"]) ?>"></a><br />
					    <?
					endif;
					if ($bDelayColumn):
					    ?>
    	...
    	<div style="clear:both;"></div>
		    <?
		    $hide_makebtn = false;
		    if (doubleval($arResult["allSum"]) < 500) {
			$hide_makebtn = true;
		    }
		    ?>
    	<div id="ORDER_MAKE_BUTTON" class="<?= $hide_makebtn ? 'hidden' : '' ?> bx_ordercart_order_pay_center">
    <? if ($arParams["USE_PREPAYMENT"] == "Y" && strlen($arResult["PREPAY_BUTTON"]) > 0): ?>
	<?= $arResult["PREPAY_BUTTON"] ?>
		    <span><?= GetMessage("SALE_OR") ?></span>
			    <? endif; ?>

    	    <a href="javascript:void(0)" class="checkout" id="add_zakaz">Быстрый заказ</a>
    	    <a href="javascript:void(0)" onclick="checkOut();" class="checkout"><?= GetMessage("SALE_ORDER") ?></a>

    	</div>
    	<div id="ORDER_MAKE_MSG"  class="<?= $hide_makebtn ? '' : 'hidden' ?>">Минимальная сумма заказа 500 руб.</div>
