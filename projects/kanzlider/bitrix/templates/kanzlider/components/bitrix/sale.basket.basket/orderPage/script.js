	...
	if (!!res.BASKET_DATA)
	{
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
                ...
                var sum = parseFloat(res.BASKET_DATA.allSum);
    if (sum >= 500)
    {
        if (BX('ORDER_MAKE_BUTTON'))
            BX('ORDER_MAKE_BUTTON').className = 'bx_ordercart_order_pay_center';

        if (BX('ORDER_MAKE_MSG'))
            BX('ORDER_MAKE_MSG').className = 'hidden';
    }
    else
    {
        if (BX('ORDER_MAKE_BUTTON'))
            BX('ORDER_MAKE_BUTTON').className = 'hidden';

        if (BX('ORDER_MAKE_MSG'))
            BX('ORDER_MAKE_MSG').className = '';
    }
