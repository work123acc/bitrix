				...
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						<?if ($arHeader["id"] == "PRICE"):?>
							<td class="price" id="col_<?=$arHeader["id"];?>_denom">
							<?echo GetMessage('KZN_BEFORE_DENOMINATION')?></td>
						<?endif;?>
							</td>
					<?
					endforeach;
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								...
								<td class="price">
										...
										<div class="current-price-denom">
											<?=CurrencyFormat($arItem["PRICE"]*$denomination,$arItem["CURRENCY"])?>
										</div>
										<div class="bxr-market-old-price-denom" id="old_price_<?=$arItem["ID"]?>_denom">
											<?if (floatval($arItem["DISCOUNT_PRICE_PERCENT"]) > 0):?>
												<?=CurrencyFormat($arItem["FULL_PRICE"]*$denomination, $arItem["CURRENCY"])?>
											<?endif;?>
										</div>
							...
							?>
								<td class="custom">
									...
									if ($arHeader["id"] == "SUM"):
									echo "<span class='sum-denomination'>".CurrencyFormat(($arItem["PRICE"]*$arItem["QUANTITY"])*$denomination, $arItem["CURRENCY"])."</span>";
									?>
										</div>
									<?
									endif;
		...
		<div class="bx_ordercart_order_pay_right">
			...
			<table class="bx_ordercart_order_sum">
				...
				<?if ($arParams["PRICE_VAT_SHOW_VALUE"] == "Y"):?>
					<tr>
						<td><?echo GetMessage('SALE_VAT_EXCLUDED')?></td>
						<td id="allSum_wVAT_DENOM"><?=CurrencyFormat($arResult["allSum"]*$denomination, $arResult["CURRENCY"])?></td>
					</tr>
					<tr>
						<td><?echo GetMessage('SALE_VAT_INCLUDED')?></td>
						<td id="allVATSum_DENOM"><?=CurrencyFormat($arResult["allVATSum"]*$denomination, $arResult["CURRENCY"])?></td>
					</tr>
				<?endif;?>

					<tr>
						<td class="fwb"><?=GetMessage("SALE_TOTAL")?></td>
						<td class="fwb" id="allSum_DENOM"><?=str_replace(" ", "&nbsp;", CurrencyFormat($arResult["allSum"]*$denomination, $arResult["CURRENCY"]))?></td>
					</tr>
					<tr>
						<td class="custom_t1"></td>
						<td class="custom_t2" style="text-decoration:line-through; color:#828282;" id="PRICE_DENOM">
							<?if (floatval($arResult["DISCOUNT_PRICE_ALL"]) > 0):?>
								<?=CurrencyFormat(($arResult["DISCOUNT_PRICE_ALL"]+$arResult["allSum"])*$denomination, $arResult["CURRENCY"])?>
							<?endif;?>
						</td>
					</tr>
