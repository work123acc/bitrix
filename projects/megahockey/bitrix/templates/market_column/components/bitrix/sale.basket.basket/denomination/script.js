...
function updateBasketTable(basketItemId, res)
{
	...
	if (!!res.BASKET_DATA)
	{
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('sum_' + id))
					BX('sum_' + id).innerHTML = item.SUM + '<span class="sum-denomination">' + formatPrice(item.PRICE*item.QUANTITY*10000) + '</span>';
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_wVAT_DENOM'))
			BX('allSum_wVAT_DENOM').innerHTML = formatPrice(res['BASKET_DATA']['allSum']*10000).replace(/\s/g, ' ');

		if (BX('allVATSum_DENOM'))
			BX('allVATSum_DENOM').innerHTML = formatPrice(res['BASKET_DATA']['allVATSum']*10000).replace(/\s/g, ' ');

		if (BX('allSum_DENOM'))
			BX('allSum_DENOM').innerHTML = formatPrice(res['BASKET_DATA']['allSum']*10000).replace(/\s/g, ' ');

		if (BX('PRICE_DENOM'))
			BX('PRICE_DENOM').innerHTML = (res['BASKET_DATA']['DISCOUNT_PRICE_ALL'] != res['BASKET_DATA']['allSum']) ? formatPrice(res['BASKET_DATA']['DISCOUNT_PRICE_ALL']*100000).replace(/\s/g, ' ') : '';
