...
if($arResult["PROPERTIES"]["BRANDS_IMG"]["VALUE"]!=""){
    
    $arSelect = Array("ID", "CODE", "DETAIL_PICTURE");
$arFilter = Array("IBLOCK_ID"=>5, "ID"=>$arResult["PROPERTIES"]["BRANDS_IMG"]["VALUE"]);
...
while($ob = $res->GetNextElement())
{
 ...
 $arResult["BRANDS_IMG"]=array(
     "HREF"=>"/brands/".$arFields["CODE"],
     "IMG"=>CFile::GetPath($arFields["DETAIL_PICTURE"])
 );
}
}else{
   $arResult["BRANDS_IMG"]["HREF"]==""; 
}
